...
public class AXMLPrinter {
	...
	public static List<String> decrypt(String tmpManifestXmlPath) {
		try {
			permissionList = new ArrayList<String>();
			AXmlResourceParser parser = new AXmlResourceParser();
			parser.open(new FileInputStream(tmpManifestXmlPath));
	...
	private static void log(String format, Object... arguments)
		...
		if (str != null) {
			int index = str.indexOf("android.permission");
			if (index != -1) {
				int start=0;
				int end=0;
				...
				if(end!=0){
					permissionList.add(str.substring(start+1,end));
				}
			}
		}
	}
